...
class PosNCFTaskDataset(Dataset):
    def __init__(self, user, pos_item_list, neg_item_list, ng_num=4):
        super(PosNCFTaskDataset, self).__init__()
        self.user = user
        self.pos_item_list = list(pos_item_list)
        self.neg_item_list = list(neg_item_list)
        self.ng_num = ng_num
    ...
    def __len__(self):
        return len(self.pos_item_list) * (self.ng_num + 1)

    def ng_sample(self):
        self.pair_list = []
        self.label_list = []

        for item in self.pos_item_list:
            self.pair_list.append([self.user, item])
            self.label_list.append(1.)
            for _ in range(self.ng_num):
                j = random.choice(self.neg_item_list)
                self.pair_list.append([self.user, j])
                self.label_list.append(0.)

    def __getitem__(self, index):
        return self.pair_list[index][0], self.pair_list[index][1], self.label_list[index] 

class NegNCFTaskDataset(Dataset):
    def __init__(self, user, pos_item_list, neg_item_list, ng_num=1):
        super(NegNCFTaskDataset, self).__init__()
        self.user = user
        self.pos_item_list = list(pos_item_list)
        self.neg_item_list = list(neg_item_list)
        self.ng_num = ng_num
    ...
    def __len__(self):
        return len(self.neg_item_list) * (self.ng_num + 1)

    def ng_sample(self):
        self.pair_list = []
        self.label_list = []

        for item in self.neg_item_list:
            self.pair_list.append([self.user, item])
            self.label_list.append(0.)
            for _ in range(self.ng_num):
                j = random.choice(self.pos_item_list)
                self.pair_list.append([self.user, j])
                self.label_list.append(1.)
...
def normal_train_one_epoch(model: nn.Module, optimizer: torch.optim, data_loader: torch.utils.data.DataLoader, device):
    ...
    for batch_data in data_loader:
        ...
        prediction = model(batch_user_list, batch_item_list)
        loss = loss_function(prediction, batch_label_list)
...
class Explainer(object):
    ...
    def init_train_process(self, user_id, update_mode="EWC", importance=500, epsilon=0.1): # update_mode in ['EWC', 'normal', 'both']
        ...
        if self.update_mode == "normal" or self.update_mode == 'both':
            ...
            self.normal_optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.normal_model.parameters()), lr=self.lr)
    ...
    def make_critique_dataloder(self, critique_attribute, test_pos):
        if test_pos:
            ...
            return build_loader(PosNCFTaskDataset(self.user_id, list(pos_item_set), list(neg_item_set)), self.batch_size)   
        else:
            pos_item_set = set(self.user_info[self.user_id])
            critique_item_set = set(self.att2item[critique_attribute])
            return build_loader(NegNCFTaskDataset(self.user_id, list(pos_item_set - critique_item_set), list(critique_item_set)), self.batch_size)
