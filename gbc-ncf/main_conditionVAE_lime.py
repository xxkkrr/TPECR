...
class predict_fn:
    def __init__(self, item_id, user_ori_embed, ori_rank, all_item_list, batch_size=128):
    ...
    def __call__(self, input_data_array):
        ...
        with torch.no_grad():
            ...
            for idx in range(0, len(input_data_tensor), self.batch_size):
                ...
                # print(f"batch_item_score_tensor: {batch_item_score_tensor}")
                ...
                for item_rank_list in batch_item_rank_list:
                    for rank, item in enumerate(item_rank_list):
                        if item == 0:
                            # print(f"rank: {rank} ori_rank: {self.ori_rank}")
...
fenduan = [(-1e10,-.5)] + [(_/100, (_+5)/100) for _ in range(-50,50,5)] + [(.5,1e10)]
