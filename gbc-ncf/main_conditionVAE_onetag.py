...
rec_model.to(device)
pretrain_recmodel_path = "./checkpoint/0428-0947/NCF-epoch33.pkl"
if use_gpu:
...
pretrain_vae_path = "./conditionvae_checkpoint/0428-1051/conditionvae5-epoch3999.pkl"
if use_gpu:
...
for (user, item), tag_diff_list in tqdm(zip(all_test_user_item, all_tag_diff_list), total=len(all_test_user_item)): 
    ...
    for idx, (tag, test_pos) in enumerate(tag_diff_list):
        ...
        if test_pos:
            ...
            cur_dataloader = build_loader(PosNCFTaskDataset(user, list(pos_item_set), list(neg_item_set)), vae_batch_size) 
        else:
            pos_item_set = set(user_info[user])
            critique_item_set = set(train_user_dataset.tag_info[tag])
            cur_dataloader = build_loader(NegNCFTaskDataset(user, list(pos_item_set - critique_item_set), list(critique_item_set)), vae_batch_size)
        user_discrete_feature, out = user_discrete_feature_update(user_discrete_feature, user_ori_embed, ori_user_discrete_feature, pre_user_ori_embed, cur_dataloader, vae_epochs, vae_lr, vae_reg1, vae_reg2, reg_update_step, vae_alpha, rec_model, my_vae)
