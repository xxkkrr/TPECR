...
def normal_train_one_epoch(model: nn.Module, optimizer: torch.optim, data_loader: torch.utils.data.DataLoader, device, user):
    model.train()
    logsigmoid = nn.LogSigmoid()
    epoch_loss = 0

    ori_embed = model.user_embeddings.weight[user].detach().clone()
    mse_loss = torch.nn.MSELoss(reduction='sum')

    for batch_data in data_loader:

        reg_loss = mse_loss(model.user_embeddings.weight[user], ori_embed)
        ...
        loss = - logsigmoid(pos_score - neg_score).sum() # + 50. * reg_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
class Explainer(object):
    ...
    def update_model(self, critique_attribute, test_pos, silence=True):
        ...
        if self.update_mode == "normal" or self.update_mode == "both":
            ...
            epoch_loss_list = []
            for _ in cur_iter:
                epoch_loss = normal_train_one_epoch(self.normal_model, self.normal_optimizer, current_dataloader, self.device, self.user_id)
                epoch_loss_list.append(epoch_loss)
            if not silence:
