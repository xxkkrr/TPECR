...
def eval_rank_attribute(origin_item_score_tensor, current_item_score_tensor, test_user, test_item_list, topk=None):
    ...
    # print(f"old_ap {old_ap}  current_ap {current_ap}")
    # print(f"old_ndcg {old_ndcg}  current_ndcg {current_ndcg}")
    # print(f"old_recall {old_recall}  curent_recall {curent_recall}")
...
class Explainer(object):
    ...
    # def make_critique_dataloder(self, critique_attribute, max_pos_limit=100, max_neg_limit=100):
    #     data_list = []
    #     current_item_score_tensor = self.get_current_item_score()
    #     score_values, score_indices = current_item_score_tensor.sort(descending=True)
    #     score_indices = score_indices.cpu().tolist()        

    #     pos_item_list = []
    #     neg_item_list = []
    #     critique_item_set = set(self.att2item[critique_attribute])
    #     for item_idx in score_indices:
    #         if item_idx in critique_item_set and len(neg_item_list) < max_neg_limit:
    #             neg_item_list.append(item_idx)
    #         if item_idx not in critique_item_set and len(pos_item_list) < max_pos_limit:
    #             pos_item_list.append(item_idx)
    #         if len(neg_item_list) == max_neg_limit and len(pos_item_list) == max_pos_limit:
    #             break

    #     for pos_i_id in pos_item_list:
    #         for neg_i_id in neg_item_list:
    #             data_list.append([self.user_id, pos_i_id, neg_i_id])
    #     return build_loader(TaskDataset(data_list), self.batch_size)
    ...
    def make_critique_dataloder(self, critique_attribute, test_pos):
        ...
        #     if item_idx in critique_item_set and len(neg_item_list) < max_neg_limit:
        #         neg_item_list.append(item_idx)
        #     if len(neg_item_list) == max_neg_limit:
        #         break

        # pos_num = max(3, 3 * len(self.user_info[self.user_id])//len(neg_item_list))
        # data_list = []
        # for neg_i_id in neg_item_list:
        #     for _ in range(pos_num):
        #         pos_i_id = random.choice(self.user_info[self.user_id])
        #         while pos_i_id in critique_item_set:
        #             pos_i_id = random.choice(self.user_info[self.user_id])
        #         data_list.append([self.user_id, pos_i_id, neg_i_id])
        # return build_loader(TaskDataset(data_list), self.batch_size)
        if test_pos:
    ...
    def update_model(self, critique_attribute, test_pos, silence=True):
        ...
        if self.update_mode == "fgsm" or self.update_mode == "both":
            ...
            # mask_data_grad = torch.zeros(data_grad.shape).to(self.device).scatter(0, data_grad.argmax(), 1.0)
            # self.fgsm_model.user_embeddings.weight.data[self.user_id] = \
            #     self.fgsm_model.user_embeddings.weight.data[self.user_id] + self.epsilon * sign_data_grad * mask_data_grad
