    ...
    def make_discrete_feature(self):
        ...
        with torch.no_grad():
            for idx in range(0, len(self.user_list), batch):
                ...
                item_score = self.recmodel.get_all_item_score(user_batch)
...
rec_model = SimpleRec(user_num, item_num, rec_hidden_dim)
rec_model.to(device)
pretrain_recmodel_path = "./checkpoint/0419-1113/SimpleRec-epoch1899.pkl"
if use_gpu:
...
pretrain_vae_path = "./conditionvae_checkpoint/0419-1416/conditionvae5-epoch3099.pkl"
if use_gpu:
...
sys.stdout = Logger(f"conditionvae5-g-result-{date_str}-num-{test_num}-topk-{topk}-att-{tag_sample_num}-attscore-onetag-pos-{test_pos}-user-{user_mode}-vae_reg1-{vae_reg1}-vae_reg2-{vae_reg2}.txt")
...
for (user, item), tag_diff_list in tqdm(zip(all_test_user_item, all_tag_diff_list), total=len(all_test_user_item)): 
    ...
    tag, test_pos = tag_diff_list[0]
    if test_pos and len(set([_ for _ in range(item_num)]) - set(train_user_dataset.tag_info[tag]) - set(user_info[user])) == 0:
        continue
    if not test_pos and len(set(user_info[user]) - set(train_user_dataset.tag_info[tag])) == 0:
        continue

    # print(f"user {user}, item {item}, tag_diff_list {tag_diff_list}")
    ...
    for idx, (tag, test_pos) in enumerate(tag_diff_list):
        ...
        cur_item_score_tensor = my_explainer.get_current_item_score(model_type='normal').cpu()
        cur_tag_score = torch.mm(cur_item_score_tensor.unsqueeze(0), train_user_dataset.tag_matrix)
        cur_tag_score = cur_tag_score / train_user_dataset.tag_len
        user_discrete_feature_gt = cur_tag_score.squeeze(0).to(torch.float32).tolist()
