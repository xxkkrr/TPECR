...
def user_discrete_feature_update(user_discrete_feature_list, user_ori_embed, ori_user_discrete_feature, pre_user_ori_embed, current_dataloader, epoch_num, lr, vae_reg1, vae_reg2, vae_reg3, reg_update_step, vae_alpha, iter_num, rec_model, my_vae):
    ...
    for _ in range(epoch_num):
        ...
        for batch_data in current_dataloader:
            ...
            # current_tag_score = current_tag_score.squeeze(0)
...
sys.stdout = Logger(f"conditionvae9-faithful-result-{date_str}-num-{test_num}-topk-{topk}-att-{tag_sample_num}-attscore-onetag-pos-{test_pos}-user-{user_mode}-vae_reg1-{vae_reg1}-vae_reg2-{vae_reg2}.txt")
print(f"baseline epochs:{vae_epochs} lr:{vae_lr} batch_size:{vae_batch_size} reg1:{vae_reg1} reg2:{vae_reg2} reg3:{vae_reg3} alpha:{vae_alpha} reg_update_step:{reg_update_step} iter_num:{iter_num}")
