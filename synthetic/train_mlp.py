...
sys.stdout = Logger(f"MLP-pretrain-{date_str}-h-{h_dim}-lr-{lr}-bs-{train_batch_size}-tagcore-2.log")
...
for _ in range(epoch_num):
    ...
    for batch_data in tqdm(train_data_loader, ncols=0):
        ...
        out = my_vae(batch_input, batch_discrete_feature)
    ...
    print(f"{time_str} train epoch {_} loss {epoch_loss} rloss {epoch_rloss}")
    if (_ + 1) % test_epoch_num == 0:
        ...
        print(f"{time_str} epoch {_} test loss {mean_loss} rloss {mean_rloss}"
